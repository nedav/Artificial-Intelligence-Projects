...
(defun next-state (s m c)
	(let* ((s_m (first s)) (s_c (second s)) (r (third s)))
	(cond 
		  ; If the action results in the number of cannibals greater than the number of missionaries (which must be nonzero) 
		  ; on the state's side of the river, return nil
		  ((and (> (- s_c c) (- s_m m)) (not (= (- s_m m) 0))) nil)
		  ; If the action results in the number of cannibals greater than the number of missionaries (which must be nonzero)
		  ; on the other side of the river, return nil
		  ((and (> (+ c (- 3 s_c)) (+ m (- 3 s_m))) (not (= (+ m (- 3 s_m)) 0))) nil)
		  ; If the number of missionaries/cannibals exceeds the number in the state
		  ((or (> m s_m) (> c s_c)) nil)
		  ; The resulting state is the m missionaries plus (3 - s_m) missionaries on the other side of the river
		  ; and c cannibals plus (3 - s_c) cannibals on the other side of the river 
		  (t (list (list (+ m (- 3 s_m)) (+ c (- 3 s_c)) (not r)))))))
...
; PATH: (initial_state, level 1 child, level 2 child, ..., leveln child)
; STATES: (child1 of leveln child, ..., childn)
...
(defun single-dfs (s path depth)
  (cond ((equal s (first path)) nil) ; If s is the initial state, path should be nil
  	))
