...
(defun test (min max your-heuristic)
    (loop for row-number upfrom min to max
        do (let ((var-name-str (format nil "P~D" row-number)))
                (format t "---------~A---------~%" var-name-str)
                (format t "h1~%")
                (time (sokoban (symbol-value (intern var-name-str)) #'h1))
                (format t "Your heuristic~%")
                (time (sokoban (symbol-value (intern var-name-str)) your-heuristic))
            )
    )
)
